...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    center_y = c1[1] + h//2
    min_length = min(w, h)
    if min_length%2 != 0 : min_length -= 1
    scale = min_length // 2
    # cv2.circle(img, (center_x, center_y), 2, (0, 0, 255), 8)
    # print("img.shape", img.shape)
    area = w*h
    c_name = label.split()[0]
    print(c_name)
    if area > 40000 and (c_name == 'person' or c_name == "car" or c_name == "motorcycle"):
        img2 = cv2.imread('red_warning.jpg')
        img2 = cv2.resize(img2, (scale, scale), interpolation=cv2.INTER_AREA)
        rows, cols, channels = img2.shape
        x_0 = center_x-scale//2
        # x_1 = center_x+scale//2
        y_0 = center_y-scale//2
        # y_1 = center_y+scale//2

        roi = img[y_0:y_0+rows, x_0:x_0+cols]
        ...
        img[y_0:y_0+rows, x_0:x_0+cols] = dst
        pass
